    ...
    def application(self, environ, start_response):
        ...
        if path == "robots.txt":
            ...
            try:
                ...
                sh['test_conf'] = qs
                self.session_conf[session.id] = sh

            return tester.display_test_list()
        elif path == '' or path == 'config':
            return tester.do_config(session.id)
            ...
            try:
                ...
                resp = SeeOther('/')
                return resp(environ, start_response)
            resp = tester.run(path, sid=session.id, **self.kwargs)
...
if __name__ == '__main__':
    ...
    kwargs = {"base_url": _base, "test_specs": fdef,
              ...
              'signing_key': keys, 'provider_cls': Provider,
